...
example [`library(nlmixr2)`](https://nlmixr2.org/) followed by
[`library(data.table)`](https://r-datatable.com)), nlmixr2save
re-attaches itself in front of data.table, so `fit := nlmixr2(...)`
keeps working. data.table's own `DT[, a := b]` is unaffected, since
data.table handles `:=` inside `[.data.table` rather than looking it up.
`` nlmixr2save::`:=` `` works regardless of the search path. Under a
`conflicts.policy` that makes conflicts errors (`"strict"`,
`"depends.ok"` or `list(error=TRUE)`, see
[`base::library()`](https://rdrr.io/r/base/library.html)) nlmixr2save
stays where it is and [`library()`](https://rdrr.io/r/base/library.html)
reports the conflict as usual.
...
#> ℹ saving fit item: mcmcAccept
#> ℹ saving fit item: mcmcAcceptCol
#> ℹ saving fit item: mcmcPhiAcf
#> ℹ saving fit item: mcmcPhiSd
#> ℹ saving fit item: mcmcStuck
#> ℹ saving fit item: message
#> ℹ saving fit item: method
#> ℹ saving fit item: mixIdx
#> ℹ saving fit item: nAGQ
