...
## saveFit errors on number
expect_error(saveFit(1), pattern = "saveFit not implemented")

## .assignParent errors on non-environment
expect_error(.assignParent(1), pattern = "env must be an environment")

if (requireNamespace("withr", quietly = TRUE)) {

  ## := with rxSolve requires seed to be set to restore
  withr::with_tempdir({
    ...
    # Pre-compile model before rxWithSeed so that the expensive rxode2 C-level
    # setup (loading the shared library, initialising the solver) happens at a
    # shallow C stack depth.  The subsequent rxSolve call inside rxWithSeed
    # then only runs the already-compiled ODE solver, which uses far less
    # C stack — avoiding overflow on platforms with a small stack limit
    # (e.g. macOS at ~8 MB).
    one.cmt <- rxode2::rxode2(one.cmt)
  ...
  ## test rxUi item saving with rxode2
  withr::with_tempdir({
...
## saveFitItem handles data.frame as expected
withr::with_tempdir({
...
if (requireNamespace("nlmixr2est", quietly = TRUE) &&
  ...
  # Helper function: compare all ui and env items of two fits.
  # In tinytest, expect_equal() records a simple result object with no
  # environment capture, so calling it from within a function or loop
  # does not risk exhausting R's node protection stack (unlike testthat
  # test_that closures which retain references to the enclosing frame).
  fitEquals <- function(fitF, fit2F) {
    fitName <- as.character(substitute(fitF))

    for (m in names(fitF$ui)) {
      if (m == "control") next
      .lbl <- paste0(fitName, "$env$ui$", m)
      if (m %in% c("mv0", "mvL")) {
        expect_equal(rxode2::rxNorm(fitF$ui[[m]]),
                     rxode2::rxNorm(fit2F$ui[[m]]),
                     info = .lbl)
      } else {
        expect_equal(fitF$ui[[m]], fit2F$ui[[m]], info = .lbl)
      }
    }
    ...
    for (n in ls(fitF$env, all.names=TRUE)) {
      ...
      if (n == "omega") {
        ...
        expect_equal(.omega, .omega2, info = .lbl)
        next
      }
      if (n %in% c("phiH", "phiC")) {
        .phiHF <- fitF$env[[n]]
      ...
      if (is.raw(fitF$env[[n]]) || is.raw(fit2F$env[[n]])) {
        ...
        expect_equal(.fit1, .fit2, info = .lbl)
      } else {
        expect_equal(fitF$env[[n]], fit2F$env[[n]], info = .lbl)
      }
    }
  }
  ...
  if (identical(Sys.getenv("NOT_CRAN"), "true")) {
    withr::with_tempdir({
      ...
      ## saving fits do not generate errors
      suppressMessages(saveFit(fitS))
      expect_true(file.exists("fitS.zip"))

      suppressMessages(saveFit(fitF, "fitF"))
      expect_true(file.exists("fitF.zip"))

      suppressMessages(saveFit(fitIF))
      expect_true(file.exists("fitIF.zip"))

      suppressMessages(saveFit(fitIS))
      ...
      ## saving fitNL does not generate errors
      suppressMessages(saveFit(fitNL, "fitNL"))
      ...
      ## saving fitNL2 does not generate errors
      suppressMessages(saveFit(fitNL2))
